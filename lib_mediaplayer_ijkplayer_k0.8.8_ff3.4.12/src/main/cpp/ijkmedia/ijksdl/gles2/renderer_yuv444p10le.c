...
static GLboolean yuv444p10le_use(IJK_GLES2_Renderer *renderer) {
    SDL_ALOGI("use render yuv420p10le\n");
...
static GLboolean yuv444p10le_uploadTexture(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay) {
    ...
    int planes[3] = {0, 1, 2};
    const GLsizei widths[3] = {overlay->pitches[0] / 2, overlay->pitches[1] / 2,
                               overlay->pitches[2] / 2};
    ...
    switch (overlay->format) {
        case SDL_FCC_I444P10LE:
            break;
        default:
            SDL_ALOGE("[yuv420p10le] unexpected format %x\n", overlay->format);
            return GL_FALSE;
    }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        glBindTexture(GL_TEXTURE_2D, renderer->plane_textures[i]);
        IJK_GLES2_checkError_TRACE("glBindTexture");
...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create_yuv444p10le() {
    SDL_ALOGI("create render yuv444p10le\n");
