...
static int
ijkio_cache_io_open(IjkURLContext *h, const char *url, int flags, IjkAVDictionary **options) {
    int ret = 0;
    IjkIOCacheContext *c = h->priv_data;
...
static int
ijkio_cache_open(IjkURLContext *h, const char *url, int flags, IjkAVDictionary **options) {
    IjkIOCacheContext *c = h->priv_data;
    int ret = 0;
    ...
    if (t) {
        c->cache_max_capacity = strtoull(t->value, NULL, 10);
    }

    t = ijk_av_dict_get(*options, "cache_file_forwards_capacity", NULL, IJK_AV_DICT_MATCH_CASE);
    if (t) {
        c->cache_file_forwards_capacity = strtoull(t->value, NULL, 10);
    }
            ...
            if (c->ijkio_app_ctx->fd >= 0) {
                ...
                if (ijk_map_size(c->cache_info_map) > 0) {
                    av_log(NULL, AV_LOG_INFO,
                           "ijkio cache will use the data that already exists\n");
                    ...
                    if (cur_exist_file_size < *c->last_physical_pos) {
                        av_log(NULL, AV_LOG_WARNING,
                               "ijkio cache exist is error, will delete last_physical_pos = %lld, cur_exist_file_size = %lld\n",
...
static int ijkio_cache_read(IjkURLContext *h, unsigned char *buf, int size) {
    ...
    if (ret != size || (!c->io_eof_reached && (c->file_logical_pos - c->read_logical_pos) <=
                                              c->cache_file_forwards_capacity)) {
        pthread_cond_signal(&c->cond_wakeup_file_background);
    }
    pthread_mutex_unlock(&c->file_mutex);
    return (int) ret;
