...
static int ijkio_urlhook_call_inject(IjkURLContext *h) {
    ...
    if (c->app_ctx) {
        ...
        if (strncmp(c->app_io_ctrl.url, "ffio:", strlen("ffio:"))) {
            snprintf(c->app_io_ctrl.url, sizeof(c->app_io_ctrl.url), "%s%s", "ffio:",
                     user_control_data.url);
        }
        ...
        av_log(NULL, AV_LOG_INFO, "%s %s (%s)\n", h->prot->name, c->app_io_ctrl.url,
               c->app_io_ctrl.is_url_changed ? "changed" : "remain");
    }
...
static int
ijkio_urlhook_init(IjkURLContext *h, const char *arg, int flags, IjkAVDictionary **options) {
    Context *c = h->priv_data;
    int ret = 0;
...
static int ijkio_urlhook_read(IjkURLContext *h, unsigned char *buf, int size) {
    ...
    if (c->test_fail_point_next > 0 && c->logical_pos >= c->test_fail_point_next) {
        av_log(NULL, AV_LOG_ERROR, "test fail point:%"
        PRId64
...
static int
ijkio_httphook_open(IjkURLContext *h, const char *arg, int flags, IjkAVDictionary **options) {
    Context *c = h->priv_data;
    int ret = 0;
    ...
    if (t) {
        c->app_ctx_intptr = (int64_t) strtoull(t->value, NULL, 10);
        c->app_ctx = (AVApplicationContext * )(intptr_t)
        c->app_ctx_intptr;
    } else {
        goto fail;
    }
    ...
    if (t) {
        c->segment_index = (int) strtoull(t->value, NULL, 10);
    }

    t = ijk_av_dict_get(*options, "ijkhttphook-test-fail-point", NULL, IJK_AV_DICT_MATCH_CASE);
    if (t) {
        c->test_fail_point = (int64_t) strtoull(t->value, NULL, 10);
    }
...
static int ijkio_httphook_read(IjkURLContext *h, unsigned char *buf, int size) {
    ...
    while ((active_reconnect || ret < 0) && c->logical_pos < c->logical_size &&
           c->abort_request == 0) {
        switch (ret) {
            case IJKAVERROR_EXIT:
                goto fail;
...
static int64_t
ijkio_httphook_reseek_at(IjkURLContext *h, int64_t pos, int whence, int force_reconnect) {
    Context *c = h->priv_data;
    int ret = 0;
...
static int64_t ijkio_httphook_seek(IjkURLContext *h, int64_t pos, int whence) {
    ...
    while (seek_ret < 0 && c->abort_request == 0) {
        ...
        av_log(NULL, AV_LOG_INFO, "%s: will reseek(%d) at pos=%"
        PRId64
        ", whence=%d\n", __func__, c->app_io_ctrl.retry_counter, pos, whence);
        seek_ret = ijkio_httphook_reseek_at(h, pos, whence, c->app_io_ctrl.is_url_changed);
        av_log(NULL, AV_LOG_INFO, "%s: did reseek(%d) at pos=%"
        PRId64
        ", whence=%d: %"
        PRId64
