...
memcpy(src_a
+ kMaxTest / 2, src_b + kMaxTest / 2, kMaxTest / 2);
...
memcpy(src_a
+ kMaxTest / 2, src_a, kMaxTest / 2);
memcpy(src_b
+ kMaxTest / 2, src_b, kMaxTest / 2);
...
* benchmark_height_);
align_buffer_64(src_b, benchmark_width_
* benchmark_height_);
for (
int i = 0;
i<benchmark_width_ * benchmark_height_;
++i) {
src_a[i] =
i;
src_b[i] =
