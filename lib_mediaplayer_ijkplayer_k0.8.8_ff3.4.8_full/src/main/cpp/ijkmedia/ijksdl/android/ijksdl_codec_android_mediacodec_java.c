...
SDL_AMediaFormat *SDL_AMediaCodecJava_getOutputFormat(SDL_AMediaCodec *thiz) {
    ...
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        SDL_ALOGE("%s: SetupThreadEnv failed", __func__);
        return NULL;
    }

    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *) thiz->opaque;
    jobject local_android_format = J4AC_MediaCodec__getOutputFormat__catchAll(env,
...
static sdl_amedia_status_t SDL_AMediaCodecJava_delete(SDL_AMediaCodec *acodec) {
    ...
    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        SDL_ALOGE("SDL_AMediaCodecJava_delete: SetupThreadEnv failed");
...
static sdl_amedia_status_t SDL_AMediaCodecJava_configure_surface(
    ...
    SDL_ALOGE("configure acodec:%p format:%p: surface:%p", android_media_codec,
              android_media_format,
              android_surface);
    J4AC_MediaCodec__configure(env, android_media_codec, android_media_format, android_surface,
                               crypto, flags);
...
static sdl_amedia_status_t SDL_AMediaCodecJava_start(SDL_AMediaCodec *acodec) {
    ...
    if (J4A_ExceptionCheck__catchAll(env)) {
        SDL_ALOGE("%s: start failed", __func__);
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
...
static ssize_t
SDL_AMediaCodecJava_writeInputData(SDL_AMediaCodec *acodec, size_t idx, const uint8_t *data,
                                   size_t size) {
    AMCTRACE("%s", __func__);
    ssize_t write_ret = -1;
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *) acodec->opaque;
    input_buffer_array = J4AC_MediaCodec__getInputBuffers__catchAll(env,
                                                                    opaque->android_media_codec);
...
ssize_t SDL_AMediaCodecJava_dequeueInputBuffer(SDL_AMediaCodec *acodec, int64_t timeoutUs) {
    ...
    //    SDL_ALOGE("%s: getInputBuffers failed", __func__);
    //    return -1;
    //}
    ...
    if (J4A_ExceptionCheck__catchAll(env)) {
        SDL_ALOGE("%s: dequeueInputBuffer failed", __func__);
...
sdl_amedia_status_t
SDL_AMediaCodecJava_queueInputBuffer(SDL_AMediaCodec *acodec, size_t idx, off_t offset, size_t size,
                                     uint64_t time, uint32_t flags) {
    AMCTRACE("%s: %d", __func__, (int) idx);

    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        SDL_ALOGE("SDL_AMediaCodecJava_queueInputBuffer: SetupThreadEnv failed");
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }

    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *) acodec->opaque;
    jobject android_media_codec = opaque->android_media_codec;
    J4AC_MediaCodec__queueInputBuffer(env, android_media_codec, (jint) idx, (jint) offset,
...
ssize_t
SDL_AMediaCodecJava_dequeueOutputBuffer(SDL_AMediaCodec *acodec, SDL_AMediaCodecBufferInfo *info,
                                        int64_t timeoutUs) {
    AMCTRACE("%s(%d)", __func__, (int) timeoutUs);
    ...
    if (!opaque->output_buffer_info) {
        opaque->output_buffer_info = J4AC_MediaCodec__BufferInfo__BufferInfo__asGlobalRef__catchAll(
                env);
        if (!opaque->output_buffer_info)
            return AMEDIACODEC__UNKNOWN_ERROR;
    }
    ...
    while (1) {
        idx = J4AC_MediaCodec__dequeueOutputBuffer(env, android_media_codec,
                                                   opaque->output_buffer_info, (jlong) timeoutUs);
        if (J4A_ExceptionCheck__catchAll(env)) {
            SDL_ALOGI("%s: Exception\n", __func__);
            return AMEDIACODEC__UNKNOWN_ERROR;
        }
        if (idx == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
            SDL_ALOGI("%s: INFO_OUTPUT_BUFFERS_CHANGED\n", __func__);
            continue;
        } else if (idx == AMEDIACODEC__INFO_OUTPUT_FORMAT_CHANGED) {
            SDL_ALOGI("%s: INFO_OUTPUT_FORMAT_CHANGED\n", __func__);
        } else if (idx >= 0) {
            AMCTRACE("%s: buffer ready (%d) ====================\n", __func__, idx);
            if (info) {
                info->offset = J4AC_MediaCodec__BufferInfo__offset__get__catchAll(env,
                                                                                  opaque->output_buffer_info);
                info->size = J4AC_MediaCodec__BufferInfo__size__get__catchAll(env,
                                                                              opaque->output_buffer_info);
                info->presentationTimeUs = J4AC_MediaCodec__BufferInfo__presentationTimeUs__get__catchAll(
                        env, opaque->output_buffer_info);
                info->flags = J4AC_MediaCodec__BufferInfo__flags__get__catchAll(env,
                                                                                opaque->output_buffer_info);
            }
        }
        break;
...
sdl_amedia_status_t
SDL_AMediaCodecJava_releaseOutputBuffer(SDL_AMediaCodec *acodec, size_t idx, bool render) {
    AMCTRACE("%s", __func__);

    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        SDL_ALOGE("%s(%d, %s): SetupThreadEnv failed", __func__, (int) idx,
                  render ? "true" : "false");
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
    ...
    if (J4A_ExceptionCheck__catchAll(env)) {
        SDL_ALOGE("%s: releaseOutputBuffer\n", __func__);
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
...
SDL_AMediaCodec *SDL_AMediaCodecJava_createByCodecName(JNIEnv *env, const char *codec_name) {
    ...
    jobject android_media_codec = J4AC_MediaCodec__createByCodecName__withCString__catchAll(env,
                                                                                            codec_name);
