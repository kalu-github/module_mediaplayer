...
static int convert_sps_pps(const uint8_t *p_buf, size_t i_buf_size,
    ...
    /* */
    if (i_data_size < 7) {
        PLAYER_ALOGE("Input Metadata too small");
        return -1;
    }
    ...
    for (unsigned int j = 0; j < 2; j++) {
        ...
        if (i_data_size < 1) {
            PLAYER_ALOGE("PPS too small after processing SPS/PPS %u",
                         i_data_size);
            return -1;
        }
        ...
        for (unsigned int i = 0; i < i_loop_end; i++) {
            if (i_data_size < 2) {
                PLAYER_ALOGE("SPS is too small %u", i_data_size);
                return -1;
            }
            ...
            if (i_data_size < i_nal_size) {
                PLAYER_ALOGE("SPS size does not match NAL specified size %u",
                             i_data_size);
                return -1;
            }
            if (i_sps_pps_size + 4 + i_nal_size > i_out_buf_size) {
                PLAYER_ALOGE("Output SPS/PPS buffer too small");
                return -1;
            }
