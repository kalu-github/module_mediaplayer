...
namespace libyuv {
    ...
    ARGBUnattenuate(orig_pixels,
    0, unatten_pixels, 0, 4, 1);
...
ARGBAttenuate(orig_pixels,
0, atten_pixels, 0, 1280, 1);
ARGBUnattenuate(atten_pixels,
0, unatten_pixels, 0, 1280, 1);
for (
int i = 0;
i<benchmark_pixels_div1280_;
++i) {
ARGBAttenuate(unatten_pixels,
0, atten2_pixels, 0, 1280, 1);
}
for (
int i = 0;
i < 1280; ++i) {
EXPECT_NEAR(atten_pixels[i * 4 + 0], atten2_pixels[i * 4 + 0],
2);
EXPECT_NEAR(atten_pixels[i * 4 + 1], atten2_pixels[i * 4 + 1],
2);
EXPECT_NEAR(atten_pixels[i * 4 + 2], atten2_pixels[i * 4 + 2],
2);
EXPECT_NEAR(atten_pixels[i * 4 + 3], atten2_pixels[i * 4 + 3],
2);
}
...
ARGBComputeCumulativeSum(&orig_pixels[0][0][0],
16 * 4,
...
x < 16; ++x) {
EXPECT_EQ((x
+ 1) * (y + 1), added_pixels[y][x][0]);
EXPECT_EQ((x
+ 1) * (y + 1) * 2, added_pixels[y][x][1]);
EXPECT_EQ((x
+ 1) * (y + 1) * 3, added_pixels[y][x][2]);
EXPECT_EQ((x
+ 1) * (y + 1) * 255, added_pixels[y][x][3]);
}
}
}
...
orig_pixels[5][3] = 224u;
// Do 16 to test asm version.
ARGBGrayTo(&orig_pixels[0][0],
0, &gray_pixels[0][0], 0, 16, 1);
...
// Do 16 to test asm version.
ARGBColorMatrix(&orig_pixels[0][0],
0, &dst_pixels_opt[0][0], 0,
&kRGBToSepia[0], 16, 1);
...
orig_pixels[3][3] = 224u;
// Do 16 to test asm version.
RGBColorMatrix(&orig_pixels[0][0],
0, &kRGBToSepia[0], 0, 0, 16, 1);
...
orig_pixels[3][3] = 3u;
// Do 16 to test asm version.
ARGBColorTable(&orig_pixels[0][0],
0, &kARGBTable[0], 0, 0, 16, 1);
...
i < 1280; ++i) {
EXPECT_EQ((i
/ 8 * 8 + 8 / 2) & 255, orig_pixels[i][0]);
EXPECT_EQ((i
/ 2 / 8 * 8 + 8 / 2) & 255, orig_pixels[i][1]);
EXPECT_EQ((i
/ 3 / 8 * 8 + 8 / 2) & 255, orig_pixels[i][2]);
EXPECT_EQ(i
& 255, orig_pixels[i][3]);
}
for (
...
ARGBMirror(&orig_pixels[0][0],
0, &dst_pixels[0][0], 0, 1280, 1);

for (
int i = 0;
i < 1280; ++i) {
EXPECT_EQ(i
& 255, dst_pixels[1280 - 1 - i][0]);
EXPECT_EQ((i
/ 2) & 255, dst_pixels[1280 - 1 - i][1]);
EXPECT_EQ((i
/ 3) & 255, dst_pixels[1280 - 1 - i][2]);
EXPECT_EQ((i
/ 4) & 255, dst_pixels[1280 - 1 - i][3]);
}
for (
int i = 0;
i<benchmark_pixels_div1280_;
...
ARGBShade(&orig_pixels[0][0],
0, &shade_pixels[0][0], 0, 8, 1, 0x80ffffff);
...
ARGBShade(&orig_pixels[0][0],
0, &shade_pixels[0][0], 0, 8, 1, 0x80808080);
...
ARGBShade(&orig_pixels[0][0],
0, &shade_pixels[0][0], 0, 8, 1, 0x10204080);
EXPECT_EQ(5u, shade_pixels[0][0]);
...
memset(orig_pixels_0,
0, sizeof(orig_pixels_0));
memset(orig_pixels_1,
0, sizeof(orig_pixels_1));
...
ARGBInterpolate(&orig_pixels_0[0][0],
0, &orig_pixels_1[0][0], 0,
...
ARGBAffineRow_C(&orig_pixels_0[0][0],
0, &interpolate_pixels_C[0][0],
uv_step, 1280);
...
SobelYRow_C(orig_pixels_0, orig_pixels_1, sobel_pixels_c,
1280);
...
i<benchmark_pixels_div1280_;
++i) {
SobelYRow(orig_pixels_0, orig_pixels_1, sobel_pixels_opt,
1280);
}
for (
int i = 0;
...
SobelRow_C(orig_sobelx, orig_sobely, sobel_pixels_c,
1280);
...
i<benchmark_pixels_div1280_;
++i) {
SobelRow(orig_sobelx, orig_sobely, sobel_pixels_opt,
1280);
}
for (
int i = 0;
...
SobelToPlaneRow_C(orig_sobelx, orig_sobely, sobel_pixels_c,
1280);
...
i<benchmark_pixels_div1280_;
++i) {
SobelToPlaneRow(orig_sobelx, orig_sobely, sobel_pixels_opt,
1280);
}
for (
int i = 0;
...
SobelXYRow_C(orig_sobelx, orig_sobely, sobel_pixels_c,
1280);
...
i<benchmark_pixels_div1280_;
++i) {
SobelXYRow(orig_sobelx, orig_sobely, sobel_pixels_opt,
1280);
}
for (
int i = 0;
...
for (
j = 0;
j<benchmark_iterations_;
j++) {
CopyPlane(orig_y
+ y_off, y_st, dst_c + y_off, stride, yw, yh);
}
...
j++) {
CopyPlane(orig_y
+ y_off, y_st, dst_opt + y_off, stride, yw, yh);
}
...
// Do 16 to test asm version.
ARGBPolynomial(&orig_pixels[0][0],
0, &dst_pixels_opt[0][0], 0,
...
align_buffer_64(lumacolortable,
32768);
int v = 0;
for (
int i = 0;
...
orig_pixels[3][3] = 224u;
// Do 16 to test asm version.
ARGBLumaColorTable(&orig_pixels[0][0],
0, &dst_pixels_opt[0][0], 0,
...
MaskCpuFlags(0);
ARGBCopyAlpha(orig_pixels, benchmark_width_
* 4,
