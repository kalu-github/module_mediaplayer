...
static GLboolean rgb565_uploadTexture(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay) {
    ...
    switch (overlay->format) {
        case SDL_FCC_RV16:
            break;
        default:
            SDL_ALOGE("[rgb565] unexpected format %x\n", overlay->format);
            return GL_FALSE;
    }
...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create_rgb565() {
    SDL_ALOGI("create render rgb565\n");
    IJK_GLES2_Renderer *renderer = IJK_GLES2_Renderer_create_base(
            IJK_GLES2_getFragmentShader_rgb());
    if (!renderer)
        goto fail;
...
static GLboolean rgb888_uploadTexture(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay) {
    ...
    switch (overlay->format) {
        case SDL_FCC_RV24:
            break;
        default:
            SDL_ALOGE("[rgb888] unexpected format %x\n", overlay->format);
            return GL_FALSE;
    }
...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create_rgb888() {
    SDL_ALOGI("create render rgb888\n");
    IJK_GLES2_Renderer *renderer = IJK_GLES2_Renderer_create_base(
            IJK_GLES2_getFragmentShader_rgb());
    if (!renderer)
        goto fail;
...
static GLboolean rgbx8888_uploadTexture(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay) {
    ...
    switch (overlay->format) {
        case SDL_FCC_RV32:
            break;
        default:
            SDL_ALOGE("[rgbx8888] unexpected format %x\n", overlay->format);
            return GL_FALSE;
    }
...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create_rgbx8888() {
    SDL_ALOGI("create render rgbx8888\n");
    IJK_GLES2_Renderer *renderer = IJK_GLES2_Renderer_create_base(
            IJK_GLES2_getFragmentShader_rgb());
    if (!renderer)
        goto fail;
