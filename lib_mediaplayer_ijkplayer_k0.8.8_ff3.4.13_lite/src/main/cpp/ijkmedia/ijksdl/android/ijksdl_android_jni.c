...
static void SDL_JNI_ThreadDestroyed(void *value) {
    ...
    if (env != NULL) {
        SDL_ALOGE("%s: [%d] didn't call SDL_JNI_DetachThreadEnv() explicity\n", __func__,
                  (int) gettid());
...
jint SDL_JNI_SetupThreadEnv(JNIEnv **p_env) {
    JavaVM *jvm = g_jvm;
    if (!jvm) {
        SDL_ALOGE("SDL_JNI_GetJvm: AttachCurrentThread: NULL jvm");
        return -1;
    }
...
int SDL_JNI_ThrowException(JNIEnv *env, const char *className, const char *msg) {
    if ((*env)->ExceptionCheck(env)) {
        ...
        if (exception != NULL) {
            SDL_ALOGW("Discarding pending exception (%s) to throw", className);
    ...
    if (exceptionClass == NULL) {
        SDL_ALOGE("Unable to find exception class %s", className);
        /* ClassNotFoundException now pending */
        goto fail;
    }

    if ((*env)->ThrowNew(env, exceptionClass, msg) != JNI_OK) {
        SDL_ALOGE("Failed throwing '%s' '%s'", className, msg);
...
int SDL_Android_GetApiLevel() {
    ...
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        SDL_ALOGE("SDL_Android_GetApiLevel: SetupThreadEnv failed");
        return 0;
    }

    SDK_INT = J4AC_android_os_Build__VERSION__SDK_INT__get__catchAll(env);
    SDL_ALOGI("API-Level: %d\n", SDK_INT);
    return SDK_INT;
#if 0
...
JNICALL SDL_JNI_OnLoad(JavaVM *vm, void *reserved) {
    int retval;
    JNIEnv *env = NULL;
