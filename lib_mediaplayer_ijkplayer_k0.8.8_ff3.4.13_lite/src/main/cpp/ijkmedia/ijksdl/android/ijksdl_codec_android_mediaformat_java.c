...
static bool
SDL_AMediaFormatJava_getInt32(SDL_AMediaFormat *aformat, const char *name, int32_t *out) {
    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        SDL_ALOGE("%s: SDL_JNI_SetupThreadEnv: failed", __func__);
        return false;
    }

    jobject android_media_format = getAndroidMediaFormat(env, aformat);
    if (!android_media_format) {
        SDL_ALOGE("%s: getAndroidMediaFormat: failed", __func__);
        return false;
    }

    jint ret = J4AC_MediaFormat__getInteger__withCString(env, android_media_format, name);
    if (J4A_ExceptionCheck__catchAll(env)) {
        SDL_ALOGE("%s: CallIntMethod: failed", __func__);
        return false;
    }
...
static void
SDL_AMediaFormatJava_setInt32(SDL_AMediaFormat *aformat, const char *name, int32_t value) {
    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        SDL_ALOGE("%s: SDL_JNI_SetupThreadEnv: failed", __func__);
        return;
    ...
    if (J4A_ExceptionCheck__catchAll(env)) {
        SDL_ALOGE("%s: CallVoidMethod: failed", __func__);
        return;
    }
}

static void SDL_AMediaFormatJava_setBuffer(SDL_AMediaFormat *aformat, const char *name, void *data,
                                           size_t size) {
    int ret = -1;
    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
    ...
    if (!opaque->android_byte_buffer) {
        opaque->android_byte_buffer = J4AC_ByteBuffer__allocateDirect__asGlobalRef__catchAll(env,
                                                                                             size);
        if (!opaque->android_byte_buffer) {
            J4A_FUNC_FAIL_TRACE();
            return;
    ...
    J4AC_MediaFormat__setByteBuffer__withCString(env, android_media_format, name,
                                                 opaque->android_byte_buffer);
    if (J4A_ExceptionCheck__catchAll(env)) {
        SDL_ALOGE("%s: call jmid_setByteBuffer: failed", __func__);
        return;
    }
}
...
SDL_AMediaFormat *
SDL_AMediaFormatJava_createVideoFormat(JNIEnv *env, const char *mime, int width, int height) {
            SDLTRACE("%s", __func__);
