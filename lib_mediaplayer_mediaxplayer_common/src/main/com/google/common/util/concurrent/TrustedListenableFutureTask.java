...
/**
 * A {@link RunnableFuture} that also implements the {@link ListenableFuture2} interface.
 *
...
class TrustedListenableFutureTask<V> extends FluentFuture.TrustedFuture<V>
  ...
  private final class TrustedFutureInterruptibleAsyncTask
      extends InterruptibleTask<ListenableFuture2<V>> {
    ...
    @Override
    ListenableFuture2<V> runInterruptibly() throws Exception {
      return checkNotNull(
          callable.call(),
    ...
    @Override
    void afterRanInterruptibly(ListenableFuture2<V> result, Throwable error) {
      if (error == null) {
        setFuture(result);
      } else {
        setException(error);
