...
static void func_free_l(SDL_Vout *vout) {
    ...
    if (opaque) {
        SDL_AMediaCodecBufferProxy **begin = (SDL_AMediaCodecBufferProxy **) ISDL_Array__begin(
                &opaque->overlay_manager);
        SDL_AMediaCodecBufferProxy **end = (SDL_AMediaCodecBufferProxy **) ISDL_Array__end(
                &opaque->overlay_manager);
        for (; begin < end; ++begin) {
...
static int func_display_overlay_l(SDL_Vout *vout, SDL_VoutOverlay *overlay, int index) {
    ...
    if (!native_window) {
        if (!opaque->null_native_window_warned) {
            ...
            SDL_ALOGW("func_display_overlay_l: NULL native_window");
        }
        return -1;
    } else {
        opaque->null_native_window_warned = 1;
    ...
    if (!overlay) {
        SDL_ALOGE("func_display_overlay_l: NULL overlay");
        return -1;
    }

    if (overlay->w <= 0 || overlay->h <= 0) {
        SDL_ALOGE("func_display_overlay_l: invalid overlay dimensions(%d, %d)", overlay->w,
                  overlay->h);
        return -1;
    }
...
static void
SDL_VoutAndroid_SetNativeWindow_l(SDL_Vout *vout, ANativeWindow *native_window, int index) {
...
SDL_VoutAndroid_obtainBufferProxy_l(SDL_Vout *vout, int acodec_serial, int buffer_index,
                                    SDL_AMediaCodecBufferInfo *buffer_info) {
    SDL_Vout_Opaque *opaque = vout->opaque;
...
SDL_VoutAndroid_obtainBufferProxy(SDL_Vout *vout, int acodec_serial, int buffer_index,
                                  SDL_AMediaCodecBufferInfo *buffer_info) {
    SDL_AMediaCodecBufferProxy *proxy = NULL;
    SDL_LockMutex(vout->mutex);
...
static int SDL_VoutAndroid_releaseBufferProxy_l(SDL_Vout *vout, SDL_AMediaCodecBufferProxy *proxy,
                                                bool render) {
    SDL_Vout_Opaque *opaque = vout->opaque;
    ...
    if (!SDL_AMediaCodec_isSameSerial(opaque->acodec, proxy->acodec_serial)) {
        SDL_ALOGW("%s: [%d] ???????? proxy %d: vout: %d idx: %d render: %s fake: %s",
                  __func__,
                  proxy->buffer_id,
                  proxy->acodec_serial,
                  SDL_AMediaCodec_getSerial(opaque->acodec),
    ...
    if (proxy->buffer_index < 0) {
        SDL_ALOGE("%s: [%d] invalid AMediaCodec buffer index %d\n", __func__, proxy->buffer_id,
                  proxy->buffer_index);
        return 0;
    ...
    sdl_amedia_status_t amc_ret = SDL_AMediaCodec_releaseOutputBuffer(opaque->acodec,
                                                                      proxy->buffer_index, render);
...
static int
SDL_VoutAndroid_releaseBufferProxy(SDL_Vout *vout, SDL_AMediaCodecBufferProxy *proxy, bool render) {
    int ret = 0;
...
int SDL_VoutAndroid_releaseBufferProxyP(SDL_Vout *vout, SDL_AMediaCodecBufferProxy **proxy,
                                        bool render) {
    int ret = 0;
...
int SDL_VoutAndroid_releaseBufferProxyP_l(SDL_Vout *vout, SDL_AMediaCodecBufferProxy **proxy,
                                          bool render) {
    int ret = 0;
