...
static void func_destroy(IJKFF_Pipeline *pipeline) {
    ...
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        PLAYER_ALOGE("amediacodec-pipeline:destroy: SetupThreadEnv failed\n");
        goto fail;
    }
...
IJKFF_Pipeline *ffpipeline_create_from_android(FFPlayer *ffp) {
    PLAYER_ALOGD("ffpipeline_create_from_android()\n");
    ...
    if (!opaque->surface_mutex) {
        PLAYER_ALOGE("ffpipeline-android:create SDL_CreateMutex failed\n");
        goto fail;
    }
