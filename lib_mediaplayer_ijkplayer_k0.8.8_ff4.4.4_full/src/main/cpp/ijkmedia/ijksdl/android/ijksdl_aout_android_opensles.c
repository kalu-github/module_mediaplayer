...
static inline SLmillibel

android_amplification_to_sles(float volumeLevel) {
...
static int aout_thread_n(SDL_Aout *aout) {
    ...
    while (!opaque->abort_request) {
        ...
        if (slRet != SL_RESULT_SUCCESS) {
            SDL_ALOGE("%s: slBufferQueueItf->GetState() failed\n", __func__);
        ...
        if (opaque->need_set_volume) {
            ...
            SDL_ALOGI("slVolumeItf->SetVolumeLevel((%f, %f) -> %d)\n", opaque->left_volume,
                      opaque->right_volume, (int) level);
            slRet = (*slVolumeItf)->SetVolumeLevel(slVolumeItf, level);
            if (slRet != SL_RESULT_SUCCESS) {
                SDL_ALOGE("slVolumeItf->SetVolumeLevel failed %d\n", (int) slRet);
                // just ignore error
            }
        }
        SDL_UnlockMutex(opaque->wakeup_mutex);
        ...
        if (opaque->need_flush) {
            ...
            if (slRet == SL_RESULT_SUCCESS) {
                ...
                SDL_ALOGE("SL_RESULT_BUFFER_INSUFFICIENT\n");
            } else {
                SDL_ALOGE("slBufferQueueItf->Enqueue() = %d\n", (int) slRet);
                break;
            }
        }
...
static int aout_open_audio(SDL_Aout *aout, const SDL_AudioSpec *desired, SDL_AudioSpec *obtained) {
    ...
    SDL_ALOGI("OpenSL-ES: native sample rate %d Hz\n", native_sample_rate);

    CHECK_COND_ERROR((desired->format == AUDIO_S16SYS), "%s: not AUDIO_S16SYS", __func__);
    CHECK_COND_ERROR((desired->channels == 2 || desired->channels == 1), "%s: not 1,2 channel",
                     __func__);
    CHECK_COND_ERROR((desired->freq >= 8000 && desired->freq <= 48000), "%s: unsupport freq %d Hz",
                     __func__, desired->freq);
    if (SDL_Android_GetApiLevel() < IJK_API_21_LOLLIPOP &&
        ...
        SDL_ALOGW("OpenSL-ES: force resample %lu to native sample rate %d\n",
    ...
    switch (desired->channels) {
        ...
        case 1:
            ...
            break;
        default:
            SDL_ALOGE("%s, invalid channel %d", __func__, desired->channels);
            goto fail;
    }
    ...
    CHECK_OPENSL_ERROR(ret,
                       "%s: slPlayerObject->GetInterface(SL_IID_ANDROIDSIMPLEBUFFERQUEUE) failed",
                       __func__);

    ret = (*opaque->slBufferQueueItf)->RegisterCallback(opaque->slBufferQueueItf,
    ...
    opaque->frames_per_buffer = opaque->milli_per_buffer * format_pcm->samplesPerSec /
    ...
    SDL_ALOGI("OpenSL-ES: bytes_per_frame  = %d bytes\n", (int) opaque->bytes_per_frame);
    SDL_ALOGI("OpenSL-ES: milli_per_buffer = %d ms\n", (int) opaque->milli_per_buffer);
    SDL_ALOGI("OpenSL-ES: frame_per_buffer = %d frames\n", (int) opaque->frames_per_buffer);
    SDL_ALOGI("OpenSL-ES: bytes_per_buffer = %d bytes\n", (int) opaque->bytes_per_buffer);
    SDL_ALOGI("OpenSL-ES: buffer_capacity  = %d bytes\n", (int) opaque->buffer_capacity);
    ...
    for (int i = 0; i < OPENSLES_BUFFERS; ++i) {
        ret = (*opaque->slBufferQueueItf)->Enqueue(opaque->slBufferQueueItf,
...
static void aout_set_volume(SDL_Aout *aout, float left_volume, float right_volume) {
    SDL_Aout_Opaque *opaque = aout->opaque;
    SDL_LockMutex(opaque->wakeup_mutex);
    SDL_ALOGI("aout_set_volume(%f, %f)", left_volume, right_volume);
