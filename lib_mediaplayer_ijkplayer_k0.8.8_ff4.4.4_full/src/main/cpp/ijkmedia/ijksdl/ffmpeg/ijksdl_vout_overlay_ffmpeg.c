...
static AVFrame *
opaque_setup_frame(SDL_VoutOverlay_Opaque *opaque, enum AVPixelFormat format, int width,
                   int height) {
...
static AVFrame *opaque_obtain_managed_frame_buffer(SDL_VoutOverlay_Opaque *opaque) {
    ...
    int frame_bytes = av_image_get_buffer_size(managed_frame->format, managed_frame->width,
                                               managed_frame->height, 1);
    AVBufferRef *frame_buffer_ref = av_buffer_alloc(frame_bytes);
    if (!frame_buffer_ref)
        return NULL;

    av_image_fill_arrays(managed_frame->data, managed_frame->linesize,
                         frame_buffer_ref->data, managed_frame->format, managed_frame->width,
                         managed_frame->height, 1);
    opaque->frame_buffer = frame_buffer_ref;
    return opaque->managed_frame;
}
...
static void func_free_l(SDL_VoutOverlay *overlay) {
    SDL_ALOGE("SDL_Overlay(ffmpeg): overlay_free_l(%p)\n", overlay);
    if (!overlay)
        return;
...
static int func_fill_frame(SDL_VoutOverlay *overlay, const AVFrame *frame) {
    ...
    switch (overlay->format) {
        ...
        case SDL_FCC_I420:
            if (frame->format == AV_PIX_FMT_YUV420P || frame->format == AV_PIX_FMT_YUVJ420P) {
                // SDL_ALOGE("direct draw frame");
                use_linked_frame = 1;
                dst_format = frame->format;
            } else {
                // SDL_ALOGE("copy draw frame");
        ...
        case SDL_FCC_RV16:
            ...
            break;
        default:
            SDL_ALOGE("SDL_VoutFFmpeg_ConvertPicture: unexpected overlay format %s(%d)",
    ...
    if (use_linked_frame) {
        ...
        if (!managed_frame) {
            SDL_ALOGE("OOM in opaque_obtain_managed_frame_buffer");
            return -1;
        }
    ...
    // swscale / direct draw
    /*
     SDL_ALOGE("ijk_image_convert w=%d, h=%d, df=%d, dd=%d, dl=%d, sf=%d, sd=%d, sl=%d",
     (int)frame->width,
     (int)frame->height,
     (int)dst_format,
    ...
    if (use_linked_frame) {
        ...
        opaque->img_convert_ctx = sws_getCachedContext(opaque->img_convert_ctx,
                                                       ...
                                                       dst_format, opaque->sws_flags, NULL, NULL,
                                                       NULL);
        if (opaque->img_convert_ctx == NULL) {
            SDL_ALOGE("sws_getCachedContext failed");
            return -1;
        }
        ...
        if (!opaque->no_neon_warned) {
            ...
            SDL_ALOGE("non-neon image convert %s(%d) -> %s(%d)", av_get_pix_fmt_name(frame->format),
                      (int) frame->format,
...
SDL_VoutOverlay *
SDL_VoutFFmpeg_CreateOverlay(int width, int height, int frame_format, SDL_Vout *display) {
    ...
    if (!overlay) {
        SDL_ALOGE("overlay allocation failed");
        return NULL;
    }
    ...
    switch (overlay_format) {
        ...
        default:
            SDL_ALOGE("SDL_VoutFFmpeg_CreateOverlay(...): unknown format %.4s(0x%x)\n",
    ...
    if (!opaque->managed_frame) {
        SDL_ALOGE("overlay->opaque->frame allocation failed\n");
        goto fail;
    }
    overlay_fill(overlay, opaque->managed_frame, opaque->planes);
