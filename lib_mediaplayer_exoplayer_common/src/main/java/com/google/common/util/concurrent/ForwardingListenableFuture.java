...
/**
 * A {@link ListenableFuture2} which forwards all its method calls to another future. Subclasses
...
public abstract class ForwardingListenableFuture<V> extends ForwardingFuture<V>
  ...
  @Override
  protected abstract ListenableFuture2<? extends V> delegate();
   ...
   * already constructed {@link ListenableFuture2} as the delegate.
   *
   * @since 9.0
   */
  public abstract static class SimpleForwardingListenableFuture<V>
      extends ForwardingListenableFuture<V> {
    private final ListenableFuture2<V> delegate;

    protected SimpleForwardingListenableFuture(ListenableFuture2<V> delegate) {
      this.delegate = Preconditions.checkNotNull(delegate);
    }

    @Override
    protected final ListenableFuture2<V> delegate() {
      return delegate;
    }
  }
}
