...
public final class ExecutionSequencer {
  ...
  private final AtomicReference<ListenableFuture2<Void>> ref =
   ...
   */
  public <T> ListenableFuture2<T> submit(final Callable<T> callable, Executor executor) {
    checkNotNull(callable);
    checkNotNull(executor);
    return submitAsync(
        new AsyncCallable<T>() {
          @Override
          public ListenableFuture2<T> call() throws Exception {
   ...
   */
  public <T> ListenableFuture2<T> submitAsync(
    ...
    final ListenableFuture2<Void> oldFuture = ref.getAndSet(newFuture);
    ...
    final ListenableFuture2<T> outputFuture = Futures.nonCancellationPropagating(taskFuture);
