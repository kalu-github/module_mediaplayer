...
//                int rotation = (videoSize.unappliedRotationDegrees > 0 ? videoSize.unappliedRotationDegrees : PlayerType.RotationType.DEFAULT);
    ...
    public void startPlayWhenReadyDelayedTime(long delayedTime) {
        try {
            ...
            if (null == mHandlerPlayWhenReadyDelayedTime) {
                mHandlerPlayWhenReadyDelayedTime = new android.os.Handler(Looper.getMainLooper()) {
                    ...
                    public void handleMessage(@NonNull Message msg) {
                        try {
                            // 起播快进
                            onEvent(PlayerType.KernelType.MEDIA_V3, PlayerType.EventType.RESUME);
                            start();
                        } catch (Exception e) {
                            LogUtil.log("VideoMedia3Player => startPlayWhenReadyDelayedTime => Exception2 " + e.getMessage());
                        } finally {
            ...
            LogUtil.log("VideoMedia3Player => startPlayWhenReadyDelayedTime => Exception1 " + e.getMessage());
        }
    }
    ...
    public void stopPlayWhenReadyDelayedTime() {
        try {
            ...
            LogUtil.log("VideoMedia3Player => playWhenReadyDelayedTime => stop mHandlerPlayWhenReadyDelayedTime");
        } catch (Exception e) {
            LogUtil.log("VideoMedia3Player => playWhenReadyDelayedTime => " + e.getMessage());
        }
    }
    ...
    public void startCheckPreparedPlaying() {
        try {
                    ...
                    public void handleMessage(@NonNull Message msg) {
                        try {
                            ...
                            // 轮询
                                ...
                                LogUtil.log("VideoMedia3Player => startCheckPreparedPlaying => loop next");
                                mHandlerPreparedPlaying.sendEmptyMessageDelayed(10001, 1000);
                            }
                        } catch (Exception e) {
                            LogUtil.log("VideoMedia3Player => startCheckPreparedPlaying => Exception2 " + e.getMessage());
                            stopCheckPreparedPlaying();
                        }
                    }
                };
            ...
            LogUtil.log("VideoMedia3Player => startCheckPreparedPlaying => Exception1 " + e.getMessage());
            stopCheckPreparedPlaying();
        }
    }
    ...
    public void stopCheckPreparedPlaying() {
        try {
            ...
            LogUtil.log("VideoMedia3Player => stopCheckPreparedPlaying => stop mHandlerPreparedPlaying");
        } catch (Exception e) {
            LogUtil.log("VideoMedia3Player => stopCheckPreparedPlaying => " + e.getMessage());
        }
    }
    ...
    public void startCheckConnectTimeout(long timeout) {
        try {
                    ...
                    public void handleMessage(@NonNull Message msg) {
                        try {
                            ...
                            // 轮询
                                ...
                                LogUtil.log("VideoMedia3Player => startCheckConnectTimeout => loop next, cast = " + cast);
                            ...
                            LogUtil.log("VideoMedia3Player => startCheckConnectTimeout => Exception2 " + e.getMessage());
            ...
            LogUtil.log("VideoMedia3Player => startCheckConnectTimeout => Exception1 " + e.getMessage());
            stopCheckConnectTimeout();
        }
    }
    ...
    public void stopCheckConnectTimeout() {
        try {
            ...
            LogUtil.log("VideoMedia3Player => stopCheckConnectTimeout => stop mHandlerConnectTimeout");
        } catch (Exception e) {
            LogUtil.log("VideoMedia3Player => stopCheckConnectTimeout => " + e.getMessage());
        }
    }
                ...
                mHandlerBufferingTimeout = new android.os.Handler(Looper.getMainLooper()) {
                    ...
                    public void handleMessage(@NonNull Message msg) {
                        ...
                        try {
                            ...
                            // 超时
                            if (cast >= timeout) {
                                onEvent(PlayerType.KernelType.MEDIA_V3, PlayerType.EventType.ERROR_BUFFERING_TIMEOUT);
                                // 1
...
//                              // 2
                            ...
                            // 轮询
                                ...
                                LogUtil.log("VideoMedia3Player => startCheckBufferingTimeout => loop next, cast = " + cast);
                                Message message = Message.obtain();
                                message.what = 10003;
                                message.obj = start;
                            ...
                            LogUtil.log("VideoMedia3Player => startCheckBufferingTimeout => " + e.getMessage());
                            stopCheckBufferingTimeout();
                        }
                    }
                };
    ...
    public void stopCheckBufferingTimeout() {
        try {
            ...
            LogUtil.log("VideoMedia3Player => stopCheckBufferingTimeout => stop mHandlerBufferingTimeout");
        } catch (Exception e) {
            LogUtil.log("VideoMedia3Player => stopCheckBufferingTimeout => " + e.getMessage());
        }
    }
}
