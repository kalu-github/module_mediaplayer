...
public final class AudioVlcPlayer extends AudioBasePlayer {
    ...
    public AudioVlcPlayer(@NonNull AudioPlayerApi playerApi, @NonNull AudioKernelApiEvent eventApi, @NonNull boolean retryBuffering) {
    ...
    public void startDecoder(@NonNull Context context, @NonNull String url, @NonNull boolean prepareAsync) {
        MPLogUtil.log("AudioVlcPlayer => startDecoder => mVlcPlayer = " + mVlcPlayer + ", url = " + url + ", prepareAsync = " + prepareAsync);
        try {
            if (null == mVlcPlayer)
                throw new Exception("mVlcPlayer error: null");
            if (url == null || url.length() == 0)
                throw new Exception("url error: " + url);
            onEvent(PlayerType.AudioKernelType.AUDIO_VLC, PlayerType.EventType.EVENT_LOADING_START);
            mVlcPlayer.setDataSource(Uri.parse(url), mPlayWhenReady);
            mVlcPlayer.play();
        } catch (Exception e) {
            onEvent(PlayerType.AudioKernelType.AUDIO_VLC, PlayerType.EventType.EVENT_LOADING_STOP);
            onEvent(PlayerType.AudioKernelType.AUDIO_VLC, PlayerType.EventType.EVENT_ERROR_URL);
        }
    }
    ...
    private void initListener() {
        MPLogUtil.log("AudioVlcPlayer => initListener =>");
        mVlcPlayerListener = new OnVlcInfoChangeListener() {
            ...
            public void onPlay() {
                onEvent(PlayerType.AudioKernelType.AUDIO_VLC, PlayerType.EventType.EVENT_LOADING_STOP);
                onEvent(PlayerType.AudioKernelType.AUDIO_VLC, PlayerType.EventType.EVENT_VIDEO_START);
            ...
            @Override
            public void onEnd() {
                onEvent(PlayerType.AudioKernelType.AUDIO_VLC, PlayerType.EventType.EVENT_VIDEO_END);
            }

            @Override
            public void onError() {
                onEvent(PlayerType.AudioKernelType.AUDIO_VLC, PlayerType.EventType.EVENT_LOADING_STOP);
                onEvent(PlayerType.AudioKernelType.AUDIO_VLC, PlayerType.EventType.EVENT_ERROR_PARSE);
            }
        };
