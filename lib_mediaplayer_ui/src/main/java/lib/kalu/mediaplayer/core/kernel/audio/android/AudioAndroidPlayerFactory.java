...
public class AudioAndroidPlayerFactory implements AudioKernelFactory<AudioAndroidPlayer> {
    ...
    @Override
    public AudioAndroidPlayer createKernel(@NonNull AudioPlayerApi playerApi, @NonNull AudioKernelApiEvent event, @NonNull boolean retryBuffering) {
        return new AudioAndroidPlayer(playerApi, event, retryBuffering);
    }
}
