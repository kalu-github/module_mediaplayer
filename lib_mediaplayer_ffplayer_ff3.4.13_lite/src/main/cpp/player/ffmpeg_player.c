...
void video_display(VideoState *is) {
    ...
    if (vp->bmp) {
        ...
        displayBmp(&is->video_player, vp->bmp, is->video_st->codec, is->video_st->codec->width,
...
int queue_picture(VideoState *is, AVFrame *pFrame, double pts) {
    ...
    if (vp->bmp) {
        ...
        updateBmp(&is->video_player, is->sws_ctx, is->video_st->codec, vp->bmp, pFrame,
                  is->video_st->codec->width, is->video_st->codec->height);
...
int stream_component_open(VideoState *is, int stream_index) {
    ...
    if (codecCtx->codec_type == AVMEDIA_TYPE_AUDIO) {
        ...
        createBufferQueueAudioPlayer(&is->audio_player, is, codecCtx->channels,
                                     codecCtx->sample_rate, is->stream_type);
    ...
    switch (codecCtx->codec_type) {
        case AVMEDIA_TYPE_AUDIO:
            ...
            av_opt_set_int(is->sws_ctx_audio, "in_sample_rate", is->audio_st->codec->sample_rate,
                           0);
            av_opt_set_int(is->sws_ctx_audio, "out_sample_rate", is->audio_st->codec->sample_rate,
                           0);
            av_opt_set_sample_fmt(is->sws_ctx_audio, "in_sample_fmt",
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            createScreen(&is->video_player, is->native_window, is->video_st->codec->width,
                         is->video_st->codec->height);
...
int decode_thread(void *arg) {
    ...
    for (;;) {
        ...
        if (is->seek_req) {
            ...
            int ret = avformat_seek_file(is->pFormatCtx, -1, seek_min, seek_target, seek_max,
                                         is->seek_flags);
            if (ret < 0) {
        ...
        if (is->audioq.size >= MAX_AUDIOQ_SIZE && !is->prepared) {
            ...
            notify_from_thread(is, MEDIA_INFO, MEDIA_INFO_VIDEO_RENDERING_START, 0);
            is->prepared = 1;
        }
...
static Uint32 notify_from_thread_cb(Uint32 interval, void *opaque) {
    ...
    if (message->is && message->is->notify_callback) {
        message->is->notify_callback(message->is->clazz, message->msg, message->ext1, message->ext2,
                                     message->from_thread);
    }
...
//
//  setDataSourceURI(&is, "/Users/xx/Desktop/BigBuckBunny_320x180.mp4", NULL);
//  prepare(&is);
//  start(&is);
//
//  for(;;) {
