...
public final class MoreExecutors {
  ...
  private static final class ScheduledListeningDecorator extends ListeningDecorator
    ...
    private static final class ListenableScheduledTask<V>
      ...
      public ListenableScheduledTask(
              ListenableFuture2<V> listenableDelegate, ScheduledFuture<?> scheduledDelegate) {
   ...
   */
  @GwtIncompatible // TODO
  private static <T> ListenableFuture2<T> submitAndAddQueueListener(
      ListeningExecutorService executorService,
      Callable<T> task,
      final BlockingQueue<Future<T>> queue) {
    final ListenableFuture2<T> future = executorService.submit(task);
    future.addListener(
        new Runnable() {
          @Override
          public void run() {
