...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
  ...
  static class Segment<K, V> extends ReentrantLock {
    ...
    V loadSync(
        ...
        throws ExecutionException {
      ListenableFuture2<V> loadingFuture = loadingValueReference.loadFuture(key, loader);
      return getAndRecordStats(key, hash, loadingValueReference, loadingFuture);
    }

    ListenableFuture2<V> loadAsync(
        final K key,
        final int hash,
        final LoadingValueReference<K, V> loadingValueReference,
        CacheLoader<? super K, V> loader) {
      final ListenableFuture2<V> loadingFuture = loadingValueReference.loadFuture(key, loader);
    ...
    V refresh(K key, int hash, CacheLoader<? super K, V> loader, boolean checkTime) {
      ...
      ListenableFuture2<V> result = loadAsync(key, hash, loadingValueReference, loader);
      if (result.isDone()) {
        try {
  ...
  static class LoadingValueReference<K, V> implements ValueReference<K, V> {
    ...
    private ListenableFuture2<V> fullyFailedFuture(Throwable t) {
    ...
    public ListenableFuture2<V> loadFuture(K key, CacheLoader<? super K, V> loader) {
      try {
        stopwatch.start();
        ...
        ListenableFuture2<V> newValue = loader.reload(key, previousValue);
        if (newValue == null) {
        ...
        ListenableFuture2<V> result = setException(t) ? futureValue : fullyFailedFuture(t);
