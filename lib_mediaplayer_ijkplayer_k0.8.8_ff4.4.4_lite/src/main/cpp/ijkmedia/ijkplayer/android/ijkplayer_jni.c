...
#define JNI_MODULE_PACKAGE      "lib/kalu/ijkplayer"
#define JNI_CLASS_IJKPLAYER     "lib/kalu/ijkplayer/IjkMediaPlayer"
...
IjkMediaPlayer_setDataSourceAndHeaders(
    ...
    PLAYER_ALOGV("setDataSource: path %s", c_path);
...
IjkMediaPlayer_setDataSourceFd(JNIEnv *env, jobject thiz, jint fd) {
    ...
    PLAYER_ALOGV("setDataSourceFd: dup(%d)=%d\n", fd, dupFd);
...
IjkMediaPlayer_setDataSourceCallback(JNIEnv *env, jobject thiz, jobject callback) {
    ...
    PLAYER_ALOGV("setDataSourceCallback: %"
                         PRId64
...
inject_callback(void *opaque, int what, void *data, size_t data_size) {
    ...
    switch (what) {
        ...
        case AVAPP_CTRL_WILL_CONCAT_SEGMENT_OPEN: {
            ...
            if (!jbundle) {
                PLAYER_ALOGE("%s: J4AC_Bundle__Bundle__catchAll failed for case %d\n", __func__,
                             what);
                goto fail;
            }
...
static bool mediacodec_select_callback(void *opaque, ijkmp_mediacodecinfo_context *mcc) {
    ...
    if (J4A_ExceptionCheck__catchAll(env) || !found_codec_name) {
        PLAYER_ALOGE("%s: onSelectCodec failed\n", __func__);
        goto fail;
    }
    ...
    while (1) {
        ...
        switch (msg.what) {
            ...
            case FFP_MSG_BUFFERING_TIME_UPDATE:
                        MPTRACE("FFP_MSG_BUFFERING_TIME_UPDATE:\n");
                break;
            ...
            case FFP_MSG_AUDIO_SEEK_RENDERING_START:
                ...
                break;
            default:
                PLAYER_ALOGE("unknown FFP_MSG_xxx(%d)\n", msg.what);
                break;
        }
        msg_free_res(&msg);
    }
...
IjkMediaPlayer_native_profileBegin(JNIEnv *env, jclass clazz, jstring libName) {
    ...
    if (s_monstartup) {
        PLAYER_ALOGW("monstartup already called\b");
        return;
    }
...
IjkMediaPlayer_native_profileEnd(JNIEnv *env, jclass clazz) {
    ...
    if (s_moncleanup) {
        PLAYER_ALOGW("moncleanu already called\b");
        return;
    }

    s_moncleanup = 1;
    moncleanup();
    PLAYER_ALOGD("moncleanup\n");
}
...
IjkMediaPlayer_setFrameAtTime(JNIEnv *env, jobject thiz, jstring path, jlong start_time,
    ...
    PLAYER_ALOGV("setFrameAtTime: path %s", c_path);
...
static JNINativeMethod g_methods[] = {
        ...
        {"_setDataSource",        "(Llib/kalu/ijkplayer/misc/IMediaDataSource;)V",      (void *) IjkMediaPlayer_setDataSourceCallback},
        {"_setAndroidIOCallback", "(Llib/kalu/ijkplayer/misc/IAndroidIO;)V",            (void *) IjkMediaPlayer_setAndroidIOCallback},
