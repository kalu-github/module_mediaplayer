...
static int func_fill_frame(SDL_VoutOverlay *overlay, const AVFrame *frame) {
    ...
    if (opaque->buffer_proxy)
        SDL_VoutAndroid_releaseBufferProxyP(opaque->vout,
                                            (SDL_AMediaCodecBufferProxy **) &opaque->buffer_proxy,
                                            false);
...
SDL_VoutOverlay *SDL_VoutAMediaCodec_CreateOverlay(int width, int height, SDL_Vout *vout) {
    ...
    if (!opaque->mutex) {
        SDL_ALOGE("SDL_CreateMutex failed");
        goto fail;
    }
...
int SDL_VoutOverlayAMediaCodec_releaseFrame_l(SDL_VoutOverlay *overlay, SDL_AMediaCodec *acodec,
                                              bool render) {
    if (!check_object(overlay, __func__))
        return -1;
