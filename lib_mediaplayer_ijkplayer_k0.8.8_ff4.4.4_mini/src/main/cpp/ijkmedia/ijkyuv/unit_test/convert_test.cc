...
ARGBToRGB565(&orig_pixels[0][0],
0, &pixels565[0][0], 0, 256, 1);
...
align_buffer_64(dst_y, kDestWidth
* kDestHeight);
...
align_buffer_64(dst_y_2, kDestWidth
* kDestHeight);
...
memset(dst_u,
2, SUBSAMPLE(kDestWidth, SUBSAMP_X) *
SUBSAMPLE(kDestHeight, SUBSAMP_Y));
memset(dst_v,
3, SUBSAMPLE(kDestWidth, SUBSAMP_X) *
SUBSAMPLE(kDestHeight, SUBSAMP_Y));
memset(dst_y_2,
1,
kDestWidth *kDestHeight
...
NV12ToI420(src_y
+
crop_y *kWidth, kWidth,
        src_uv
+ (crop_y / 2) * kWidth, kWidth,
dst_y, kDestWidth,
...
for (
int i = 0;
i < SUBSAMPLE(kDestHeight, SUBSAMP_Y); ++i) {
for (
int j = 0;
j < SUBSAMPLE(kDestWidth, SUBSAMP_X); ++j) {
