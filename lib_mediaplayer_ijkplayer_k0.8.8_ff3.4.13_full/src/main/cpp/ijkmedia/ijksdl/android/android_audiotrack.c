...
void
SDL_Android_AudioTrack_set_volume(JNIEnv *env, SDL_Android_AudioTrack *atrack, float left_volume,
                                  float right_volume) {
    J4AC_AudioTrack__setStereoVolume__catchAll(env, atrack->thiz, left_volume, right_volume);
}

SDL_Android_AudioTrack *
SDL_Android_AudioTrack_new_from_spec(JNIEnv *env, SDL_Android_AudioTrack_Spec *spec) {
    assert(spec);

    switch (spec->channel_config) {
        case CHANNEL_OUT_MONO:
            SDL_ALOGI("SDL_Android_AudioTrack: %s", "CHANNEL_OUT_MONO");
            break;
        case CHANNEL_OUT_STEREO:
            SDL_ALOGI("SDL_Android_AudioTrack: %s", "CHANNEL_OUT_STEREO");
            break;
        default:
            SDL_ALOGE("%s: invalid channel %d", __func__, spec->channel_config);
            return NULL;
    }

    switch (spec->audio_format) {
        case ENCODING_PCM_16BIT:
            SDL_ALOGI("SDL_Android_AudioTrack: %s", "ENCODING_PCM_16BIT");
            break;
        case ENCODING_PCM_8BIT:
            SDL_ALOGI("SDL_Android_AudioTrack: %s", "ENCODING_PCM_8BIT");
            break;
#if 0
            case ENCODING_PCM_FLOAT:
                SDL_ALOGI("SDL_Android_AudioTrack: %s", "ENCODING_PCM_FLOAT");
                if (sdk_int < IJK_API_21_LOLLIPOP) {
                    SDL_ALOGI("SDL_Android_AudioTrack: %s need API 21 or above", "ENCODING_PCM_FLOAT");
                    return NULL;
                }
                break;
#endif
        default:
            SDL_ALOGE("%s: invalid format %d", __func__, spec->audio_format);
            return NULL;
    }

    if (spec->sample_rate_in_hz <= 0) {
        SDL_ALOGE("%s: invalid sample rate %d", __func__, spec->sample_rate_in_hz);
        return NULL;
    }

    SDL_Android_AudioTrack *atrack = (SDL_Android_AudioTrack *) mallocz(
            sizeof(SDL_Android_AudioTrack));
    if (!atrack) {
        SDL_ALOGE("%s: mallocz faild.\n", __func__);
        return NULL;
    }
    atrack->spec = *spec;
    ...
    if (J4A_ExceptionCheck__catchAll(env) || min_buffer_size <= 0) {
        SDL_ALOGE("%s: J4AC_AudioTrack__getMinBufferSize: return %d:", __func__, min_buffer_size);
        free(atrack);
        return NULL;
    }
    ...
    SDL_ALOGI("%s: init volume as %f/(%f,%f)", __func__, init_volume, atrack->min_volume,
              atrack->max_volume);
...
SDL_Android_AudioTrack *
SDL_Android_AudioTrack_new_from_sdl_spec(JNIEnv *env, const SDL_AudioSpec *sdl_spec) {
...
void
SDL_Android_AudioTrack_get_target_spec(SDL_Android_AudioTrack *atrack, SDL_AudioSpec *sdl_spec) {
...
int SDL_Android_AudioTrack_reserve_byte_buffer(JNIEnv *env, SDL_Android_AudioTrack *atrack,
                                               int size_in_byte) {
...
int SDL_Android_AudioTrack_write(JNIEnv *env, SDL_Android_AudioTrack *atrack, uint8_t *data,
                                 int size_in_byte) {
    if (size_in_byte <= 0)
        return size_in_byte;
    ...
    int retval = J4AC_AudioTrack__write(env, atrack->thiz, atrack->byte_buffer, 0,
                                        (int) size_in_byte);
    if (J4A_ExceptionCheck__catchAll(env))
        return -1;
