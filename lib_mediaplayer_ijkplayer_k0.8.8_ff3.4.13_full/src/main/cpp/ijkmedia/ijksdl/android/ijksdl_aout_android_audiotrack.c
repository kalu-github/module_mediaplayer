...
//                SDL_CondWaitTimeout(opaque->wakeup_cond, opaque->wakeup_mutex, 1000);
        ...
        if (opaque->need_flush) {
            ...
            if (written != copy_size) {
                SDL_ALOGW("AudioTrack: not all data copied %d/%d", (int) written, (int) copy_size);
            }
        }
...
static int aout_thread(void *arg) {
    ...
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        SDL_ALOGE("aout_thread: SDL_AndroidJni_SetupEnv: failed");
        return -1;
    }
...
static int aout_open_audio_n(JNIEnv *env, SDL_Aout *aout, const SDL_AudioSpec *desired,
    ...
    if (!opaque->atrack) {
        SDL_ALOGE("aout_open_audio_n: failed to new AudioTrcak()");
        return -1;
    }

    opaque->buffer_size = SDL_Android_AudioTrack_get_min_buffer_size(opaque->atrack);
    if (opaque->buffer_size <= 0) {
        SDL_ALOGE("aout_open_audio_n: failed to getMinBufferSize()");
    ...
    if (!opaque->buffer) {
        SDL_ALOGE("aout_open_audio_n: failed to allocate buffer");
        SDL_Android_AudioTrack_free(env, opaque->atrack);
        opaque->atrack = NULL;
        return -1;
    }
    ...
    opaque->audio_tid = SDL_CreateThreadEx(&opaque->_audio_tid, aout_thread, aout,
                                           "ff_aout_android");
    if (!opaque->audio_tid) {
        SDL_ALOGE("aout_open_audio_n: failed to create audio thread");
        SDL_Android_AudioTrack_free(env, opaque->atrack);
        opaque->atrack = NULL;
        return -1;
    }
...
static int aout_open_audio(SDL_Aout *aout, const SDL_AudioSpec *desired, SDL_AudioSpec *obtained) {
    ...
    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        SDL_ALOGE("aout_open_audio: AttachCurrentThread: failed");
        return -1;
    }
