...
static void ijkio_manager_parse_cache_info(IjkIOApplicationContext *app_ctx, char *file_path) {
    ...
    while (!feof(fp)) {
        ...
        if (ijk_av_strstart(string_line, "tree_index:", (const char **) ptr)) {
            ...
            tree_physical_init_pos = strtoull(*ptr, NULL, 10);
            ...
            tree_physical_size = strtoull(*ptr, NULL, 10);
            ...
            tree_file_size = strtoull(*ptr, NULL, 10);
            ...
            entry_logical_pos = strtoull(*ptr, NULL, 10);
            ...
            entry_physical_pos = strtoull(*ptr, NULL, 10);
            ...
            entry_size = strtoull(*ptr, NULL, 10);
...
int ijkio_manager_io_open(IjkIOManagerContext *h, const char *url, int flags,
    ...
    if (inner) {
        ...
        if (h->ijk_ctx_map) {
            ...
            ijk_map_put(h->ijk_ctx_map, (int64_t)(intptr_t)
            h->cur_ffmpeg_ctx, inner);
        }
    ...
    if (inner) {
        ...
        if (h->ijk_ctx_map) {
            ijk_map_remove(h->ijk_ctx_map, (int64_t)(intptr_t)
            h->cur_ffmpeg_ctx);
        }
...
int ijkio_manager_io_read(IjkIOManagerContext *h, unsigned char *buf, int size) {
    ...
    IjkURLContext *inner = ijk_map_get(h->ijk_ctx_map, (int64_t)(intptr_t)
    h->cur_ffmpeg_ctx);
